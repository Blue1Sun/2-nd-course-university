#include <stdio.h>
#include <string.h>
#include <ctype.h> //дл€ isdigit

using namespace std;
#define FCLR while (getc(fin)!='\n')
//очистка входной строки

//вложенна€ структура (1)
struct model
{
int seriya,number;
};

//вложенна€ структура (2)
struct date
{
int year, month;
};
...
struct car
{model mod; //вложенна€ структура (1)
date dat; //вложенна€ структура (2)
...
//ћодели, у которых имеетс€ максимальное количество различных объемов двигател€.
...
{car *iukaz,*jukaz;
    ...
    *n1=-1;//изначально нет моделей
    ...
    {for(iukaz=cat;iukaz<cat+kol-1;iukaz++)//высчитываем максимум разных обьемов
        ...
        if (diff>max) max=diff; //вычисл€ем максимум
        }
